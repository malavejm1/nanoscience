...
implicit none
 ...
 ! R2=0.d0
  ...
  e_p=0.d0
  rr=sqrt((R1(1)-R2(1))**2+(R1(2)-R2(2))**2+(R1(3)-R2(3))**2)
  e_p=e_p+9.d0/rr
  do i=1,N_particle
    rr=sqrt((r(1,i)-R1(1))**2+(r(2,i)-R1(2))**2+(r(3,i)-R1(3))**2)
    e_p=e_p-3.d0/rr
    rr=sqrt((r(1,i)-R2(1))**2+(r(2,i)-R2(2))**2+(r(3,i)-R2(3))**2)
    e_p=e_p-3.d0/rr
  end do
  do i=1,N_particle
    do j=i+1,N_particle
...
function ylm(rv,l,m)
  ...
  subroutine wave_fun(p,par,wf)
    ...
    call wf_Li2(p,par,wf)

  end subroutine wave_fun

subroutine wf_Li2(p,par,wf)
implicit none
real*8             :: wave_function
real*8             :: fact(0:4),rr,phi_up(3,3),phi_down(3,3),C_basis(8,3),xi(8),d_up,d_down,pc(3)
integer            :: i,j,nb(10),lb(10),mb(10),spin(10)
real*8             :: a,b,rij,r2x,r2y,r2z,jf,wave_fun,w,rc(3),d
integer            :: k,ij,NN=8,f(0:4),kk
double precision :: p(3,n_particle), par(n_params), wf(n_basis)
real*8 :: R1(3),R2(3)
...
!c_basis=0.d0
!c_basis(1,1)=0.606630
!c_basis(2,1)=1.0
!c_basis(3,1)=0.0
!c_basis(4,1)=0.061592
!c_basis(5,1)=0.606630
!c_basis(6,1)=1.0
!c_basis(7,1)=0.0
!c_basis(8,1)=-0.061592
!c_basis(1,2)=0.603086
!c_basis(2,2)=1.0
!c_basis(3,2)=0.0
!c_basis(4,2)=0.002946
!c_basis(5,2)=-0.603086
!c_basis(6,2)=-1.0
!c_basis(7,2)=0.0
!c_basis(8,2)=0.002946
!c_basis(1,3)=0.105957
!c_basis(2,3)=0.0
!c_basis(3,3)=1.0
!c_basis(4,3)=0.305729
!c_basis(5,3)=0.105957
!c_basis(6,3)=0.0
!c_basis(7,3)=1.0
!c_basis(8,3)=-0.305729
...
c_basis=0.d0
c_basis(1,1)=par((kk-1)*n_par+2) 
c_basis(2,1)=1.0
c_basis(3,1)=0.0
c_basis(4,1)=par((kk-1)*n_par+3) 
c_basis(5,1)=par((kk-1)*n_par+4) 
c_basis(6,1)=1.0
c_basis(7,1)=0.0
c_basis(8,1)=par((kk-1)*n_par+5) 
c_basis(1,2)=par((kk-1)*n_par+6) 
c_basis(2,2)=1.0
c_basis(3,2)=0.0
c_basis(4,2)=par((kk-1)*n_par+7) 
c_basis(5,2)=par((kk-1)*n_par+8) 
c_basis(6,2)=-1.0
c_basis(7,2)=0.0
c_basis(8,2)=par((kk-1)*n_par+9) 
c_basis(1,3)=par((kk-1)*n_par+10) 
c_basis(2,3)=0.0
c_basis(3,3)=1.0
c_basis(4,3)=par((kk-1)*n_par+11) 
c_basis(5,3)=par((kk-1)*n_par+12) 
c_basis(6,3)=0.0
c_basis(7,3)=1.0
c_basis(8,3)=par((kk-1)*n_par+13) 



xi(1)=par((kk-1)*n_par+14) 
xi(2)=par((kk-1)*n_par+15) 
xi(3)=par((kk-1)*n_par+16) 
xi(4)=par((kk-1)*n_par+17) 
xi(5)=par((kk-1)*n_par+14)
xi(6)=par((kk-1)*n_par+15)
xi(7)=par((kk-1)*n_par+16)
xi(8)=par((kk-1)*n_par+17)
...
do k=1,6
  do i=1,3
    ...
    do j=1,NN
      if(j.le.4) then
        ...
        rr=sqrt(pc(1)**2+pc(2)**2+pc(3)**2)
      w=w+C_basis(j,i)*sqrt((2*xi(j))**(2*nb(j)+1)/f(2*nb(j)))*rr**(nb(j)-1)*exp(-xi(j)*rr)*ylm(pc(:),lb(j),mb(j))
    end do
    if(k.le.3) then
      phi_up(k,i)=w
    else
 ...
 call det(phi_up,3,d_up)
 call det(phi_down,3,d_down)
 wf(kk)=jf*d_up*d_down*d**photons(kk)
enddo

end subroutine wf_Li2
